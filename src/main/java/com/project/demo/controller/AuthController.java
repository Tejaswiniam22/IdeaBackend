...
import com.project.demo.security.LoginAttemptService;
...
// 🚨 Check if IP is blocked
        if (loginAttemptService.isBlocked(ipAddress)) {
            log.warn("Blocked login attempt from IP: {}", ipAddress);
            return ResponseEntity.status(403).body(Map.of("error", "Too many failed attempts. Try again after 10 minutes."));
        }
